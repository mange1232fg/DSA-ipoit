...
public class B_Sheduler {
    ...
    List<Event> calcStartTimes(Event[] events, int from, int to) {
        ...
        // Отфильтруем события, которые выходят за пределы [from, to]
        ...
        for (Event e : filtered) {
            if (e.start >= currentTime) {  // событие не пересекается с предыдущим
                result.add(e);
                currentTime = e.stop;       // обновляем текущее время
            }
        }
